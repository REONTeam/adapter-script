...
# Version 1.4.1
#
# Original script by Háčky.
# BGBLinkCable class by TheZZAZZGlitch.
# IP Filter based on Login by eintei95.
# Other changes by Arves100
#
# Version 1.4.1: Rewritten email filter
...
# Ensure your config file is correctly setup!
...
AcceptedHeaders = [ b"Date", b"X-Game-title", b"Content-Type", b"From", b"X-Game-result", b"Subject", b"X-Game-code", b"X-GBmail-type", b"To" ]

# New implementation of email filter
# Translates real server email to mobile trainer compatible emails
def FilterEmail(text):
    if len(text) < 1:
        return b"-ERR Invalid email"

    output = b""
    for line in text.split(b"\r\n"):
        if any(elem in line for elem in AcceptedHeaders):
            output += line + b"\r\n"
        elif len(line) < 1:
            break

    if not b"Date:" in output:
        rvp = text.find(b"Received: from ")
        if rvp == -1:
            return b"-ERR Invalid or unsupported date format\r\n"
        ...
        output += b'Date: ' + text[pev+2:eod] + b'\r\n'

    rn2 = text.find(b"\r\n\r\n")
    if rn2 == -1:
        return b"-ERR Missing double rn"
		
    fdo = text.find(b"\r\n.\r\n",rn2)
    if fdo == -1:
        return b"-ERR Missing dot rn"
    # Fix issue for dovecot?
    fdx = text.find(b"\r\n\r\n.\r\n", rn2)

    output += text[rn2+2:fdo]
...
# Argument one (received_byte) is the byte that the GameBoy have sent
...
# This function generate the content that will be sent to the adapter
def craftResponsePacket():
    global packet_data, configuration_data, pop_start_command_sent, is_line_busy, dion_login_ip, dion_login_port, enable_external_dns_server, enable_dion_login_server, external_server_port, http_session_started, response_text, pop_session_started, smtp_session_started, external_server_socket, p2p_destination_ip
    ...
    if(packet_data['id'] == 0x10): # Command 0x10: NINTENDO
        ...
        if expecting_ip == None and enable_external_dns_server == True:
            try:
                ...
                print('<< Domain %s does not exists, replacing to default' % (packet_data['data'].decode()))
                expecting_ip = backup_ip
    ...
    if verbose_mode == True:
        print("Length of bytes:",packet_data['size'])
...
def craftExternalPOPResponse():
    global packet_data, response_text, external_server_socket, configuration_data, real_email_domain
    ...
    if(len(response_text) == 0):
        ...
        # This code remove the unused headers and append the headers that the mobile trainer require to receive an email
             while not b'\r\n.\r\n' in response_text:
                 response_text += external_server_socket.recv(4096)
             response_text = FilterEmail(response_text)
    
 
        if not response_text or response_text == b"E_FAIL":
...
# This function connect to an external HTML server and process the data
def craftExternalHTMLResponse():
    global packet_data, http_text, http_session_started, response_text, external_server_socket
    if(len(response_text) == 0):
        if(len(packet_data['data']) > 1):
